...
if [[ " ${debian[*]} " =~ " $os_distro " ]]; then
    ...
    chown apache.apache ../modules/data_release/user_uploads
    chown apache.apache ../smarty/templates_c
    # Make Apache the group for project directory, so that the web based install
    # can write the config.xml file.
    chgrp apache ../project
    chmod 770 ../project
else
    ...
    while true; do
        ...
        case $yn in
            [Yy]* )
                ...
                # Need to pipe to tee because > is done as the logged in user, even if run through sudo
