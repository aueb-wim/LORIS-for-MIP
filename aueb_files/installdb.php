...
tplvar('dbname', 'LORIS');
tplvar('lorismysqluser', 'lorisuser');
tplvar('lorismysqlpassword', 'neopass');
tplvar('frontenduser', 'dfuser');
tplvar('frontendpassword', 'neopass');
tplvar('use_existing_database', false);
tplvar('use_existing_tables', false);
tplvar('use_existing_configs', false);
tplvar('lorismysql_already_created', false);
...
if ($installer->createMySQLDB($tpl_data) === false) {
...
if ($installer->sourceSchema($tpl_data) === false) {
    $tpl_data['error'] = $installer->getLastError();
    $tpl_data['Page']  = "";
    exit(1);
}
else {
    echo 'SourceSchema ok';
}
if ($installer->updateBaseConfig($tpl_data) === false) {
    $tpl_data['error'] = $installer->getLastError();
    $tpl_data['Page']  = "";
    exit(1);
}
else {
    echo 'updateBaseConfig ok';
}
if ($installer->createMySQLAccount($tpl_data) === false) {
    $tpl_data['error'] = $installer->getLastError();
    $tpl_data['Page']  = "MySQLUserPrompt";
    exit(1);
}
else {
    echo 'createMySQLAccount ok';
}
if ($installer->resetFrontEndAdmin($tpl_data) === false) {
    $tpl_data['error'] = $installer->getLastError();
    $tpl_data['Page']  = "MySQLUserPrompt";
    exit(1);
}
else {
    echo 'resetFrontEndAdmin ok';
}
if ($installer->configWritable()) {
    if ($installer->writeConfig($tpl_data) === false) {
        $tpl_data['error'] = $installer->getLastError();
        $tpl_data['Page']  = "MySQLUserPrompt";
        exit(1);
    }
    ...
    $tpl_data['configcontent']  = htmlspecialchars(
        $installer->getConfigContent($tpl_data)
    );
}
$tpl_data['Page'] = "Done";
